...
- **视频讲解**：[90 秒英文讲解](https://gnuser.github.io/codex-usage-meter/) · [下载字幕](media/product-story-cache.srt)

[![安装与使用讲解视频](media/product-story-cache.png)](https://gnuser.github.io/codex-usage-meter/)
...
- **逐轮用量**：每个会话显示本轮、累计 token、本轮输入 / 输出和最近 10 轮柱状图；K / M / B / T 单位，四舍五入显示整数。
...
- 点击会话右侧 ▸ 展开（默认收起），再点 ▾ 收起：查看该会话按模型累计的 input / output tokens，使用 K、M、B 单位并四舍五入显示整数；无法归属到具体模型的记录单独列出。
...
macOS 先点 Dock 中的「Codex Usage」；Windows 先检查任务栏及托盘折叠菜单。仍找不到时，重新运行本节前面的 `floating.py show --thread 实际ID`，会恢复共用窗口。
...
更多说明见 [能力与边界](CAPABILITIES.zh-CN.md)。
...
HTTP 测试需要监听本机临时端口。历史版本验证见 [测试记录](VALIDATION.zh-CN.md)，辅助工具来源见 [工具来源](../tools/PROVENANCE.zh-CN.md)。
...
Tibo 不随标准安装包提供，默认关闭。安装时使用 `--with-tibo`，或源码启动服务前设置 `CODEX_USAGE_TIBO=1`，才会启用。
