...
- [Installation guide](docs/INSTALL.md): macOS, Windows, and optional integrations.
- [Usage guide](docs/USAGE.md): controls, numbers, and troubleshooting.
- [29-second English overview](docs/media/getting-started.mp4) · [English subtitles](docs/media/getting-started.srt).

[![Watch the product overview](docs/media/getting-started.png)](docs/media/getting-started.mp4)
...
For a persistent window, follow the [macOS or Windows installation instructions](docs/INSTALL.md#floating-window). Keep the source directory: the installed window and hooks depend on its scripts.
...
Replace `YOUR_CODEX_THREAD_ID` with an actual ID; see [manual opening](docs/INSTALL.md#open-manually).
...
Skip this if you only need the window or browser dashboard. Follow the [plugin installation guide](docs/INSTALL.md#optional-codex-plugin).
...
| --- | --- |
| Window is missing | Check Dock/menu bar or taskbar/hidden tray icons; then use [manual opening](docs/INSTALL.md#open-manually) |
...
See [capabilities and limitations](docs/CAPABILITIES.md).
...
HTTP tests need a local listening port. See [validation history](docs/VALIDATION.md) and [helper provenance](tools/PROVENANCE.md).
...
Follow the [Chrome setup instructions](docs/INSTALL.md#optional-tibo-monitor). The monitor is free and requires the repository's extension; merely signing into Chrome is not enough. It reads the latest three original posts from [Tibo](https://x.com/thsottiaux), excluding replies, self-replies, reposts, recognized pins, and other authors. Quoted content is not treated as his own statement.
